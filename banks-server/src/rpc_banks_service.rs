//! The `rpc_banks_service` module implements the Analog Banks RPC API.

use {
    crate::banks_server::start_tcp_server,
    futures::{future::FutureExt, pin_mut, prelude::stream::StreamExt, select},
    analog_runtime::{bank_forks::BankForks, commitment::BlockCommitmentCache},
    std::{
        net::SocketAddr,
        sync::{
...
impl RpcBanksService {
    ...
    pub fn new(
        ...
        let thread_hdl = Builder::new()
            .name("analog-rpc-banks".to_string())
            .spawn(move || {
                Self::run(
                    listen_addr,
                    tpu_addr,
