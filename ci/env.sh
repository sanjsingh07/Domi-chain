...
if [[ -n $CI ]]; then
  ...
  if [[ -n $TRAVIS ]]; then
    ...
    # to how analog-ci-gate is used to trigger PR builds rather than using the
    ...
    if [[ -n $BUILDKITE_TRIGGERED_FROM_BUILD_PIPELINE_SLUG ]]; then
      # The analog-secondary pipeline should use the slug of the pipeline that
      # triggered it
