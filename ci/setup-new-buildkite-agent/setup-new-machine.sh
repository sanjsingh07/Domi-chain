...
cat >/etc/apt/apt.conf.d/99-analog <<'EOF'
...
"$ANALOG_ROOT"/net/scripts/install-docker.sh
usermod -aG docker "$SETUP_USER"
"$ANALOG_ROOT"/net/scripts/install-certbot.sh
...
"$ANALOG_ROOT"/net/scripts/install-earlyoom.sh
"$ANALOG_ROOT"/net/scripts/install-nodejs.sh
"$ANALOG_ROOT"/net/scripts/localtime.sh
"$ANALOG_ROOT"/net/scripts/install-redis.sh
"$ANALOG_ROOT"/net/scripts/install-rsync.sh
"$ANALOG_ROOT"/net/scripts/install-libssl-compatability.sh
