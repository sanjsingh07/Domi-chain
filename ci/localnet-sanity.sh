...
killNodes() {
  ...
  echo "--- RPC exit"
  $analog_validator --ledger "$SOLANA_CONFIG_DIR"/bootstrap-validator exit --force || true
  $analog_validator --ledger "$SOLANA_CONFIG_DIR"/validator exit --force || true
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $analog_ledger_tool --ledger "$SOLANA_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
