...
startNodes() {
  ...
  for i in $(seq 0 $((${#nodes[@]} - 1))); do
    ...
    if [[ "$i" -eq 1 ]]; then
      ...
      (
        set -x
        $analog_cli --keypair config/bootstrap-validator/identity.json \
...
killNodes() {
  ...
  echo "--- RPC exit"
  $analog_validator --ledger "$ANALOG_CONFIG_DIR"/bootstrap-validator exit --force || true
  $analog_validator --ledger "$ANALOG_CONFIG_DIR"/validator exit --force || true
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $analog_ledger_tool --ledger "$ANALOG_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
...
while [[ $iteration -le $iterations ]]; do
  ...
  (
    ...
    $analog_keygen new --no-passphrase -fso $client_keypair || exit $?
    $analog_gossip spy -n 127.0.0.1:8001 --num-nodes-exactly $numNodes || exit $?
