...
```bash
analog-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
analog-tokens transaction-log --output-path transactions.csv
```
...
```bash
analog-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
analog-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
analog-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
    --input-csv <ALLOCATIONS_CSV> \
    --stake-authority <KEYPAIR> --withdraw-authority <KEYPAIR> --fee-payer <KEYPAIR>
```

Currently, this will subtract 1 ANLOG from each allocation and store it the
recipient address. That ANLOG can be used to pay transaction fees on staking
...
Distributing SPL Tokens works very similarly to distributing ANLOG, but requires
...
```bash
analog-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
analog-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
As with ANLOG, you can List the differences between a list of expected
distributions and the record of what transactions have already been sent using
the `--dry-run` parameter, or `analog-tokens balances`.
...
```bash
analog-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
