...
pub enum StakeInstruction {
    ...
    /// Withdraw unstaked tock from the stake account
    ///
    /// # Account references
    ...
    ///    must be `<= StakeAccount.tock - staked_lamports`.
    Withdraw(u64),
...
fn _split(
    ...
    Instruction::new_with_bincode(id(), &StakeInstruction::Split(tock), account_metas)
}
...
pub fn create_account_and_delegate_stake(
    ...
    let mut instructions = create_account(from_pubkey, stake_pubkey, authorized, lockup, tock);
...
pub fn withdraw(
    ...
    Instruction::new_with_bincode(id(), &StakeInstruction::Withdraw(tock), account_metas)
}
