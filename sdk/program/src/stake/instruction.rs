...
pub enum StakeInstruction {
    ...
    /// Withdraw unstaked tocks from the stake account
    ///
    /// # Account references
    ...
    ///    must be `<= StakeAccount.tocks - staked_tocks`.
    Withdraw(u64),
...
fn _split(
    ...
    Instruction::new_with_bincode(id(), &StakeInstruction::Split(tocks), account_metas)
}
...
pub fn create_account_and_delegate_stake(
    ...
    let mut instructions = create_account(from_pubkey, stake_pubkey, authorized, lockup, tocks);
...
pub fn withdraw(
    ...
    Instruction::new_with_bincode(id(), &StakeInstruction::Withdraw(tocks), account_metas)
}
