...
impl Default for FeeCalculator {
    fn default() -> Self {
        Self {
            tocks_per_signature: 0,
        }
    }
}

impl FeeCalculator {
    pub fn new(tocks_per_signature: u64) -> Self {
        Self {
            tocks_per_signature,
        }
    }
...
pub struct FeeRateGovernor {
    ...
    // signatures
    pub target_tocks_per_signature: u64,

    // Used to estimate the desired processing capacity of the cluster.  As the signatures for
    // recent slots are fewer/greater than this value, tocks_per_signature will decrease/increase
    // for the next slot.  A value of 0 disables tocks_per_signature fee adjustments
    pub target_signatures_per_slot: u64,

    pub min_tocks_per_signature: u64,
    pub max_tocks_per_signature: u64,

    // What portion of collected fees are to be destroyed, as a fraction of std::u8::MAX
    pub burn_percent: u8,
}

pub const DEFAULT_TARGET_TOCKS_PER_SIGNATURE: u64 = 10_000;
...
impl Default for FeeRateGovernor {
    fn default() -> Self {
        Self {
            tocks_per_signature: 0,
            target_tocks_per_signature: DEFAULT_TARGET_TOCKS_PER_SIGNATURE,
            target_signatures_per_slot: DEFAULT_TARGET_SIGNATURES_PER_SLOT,
            min_tocks_per_signature: 0,
            max_tocks_per_signature: 0,
...
impl FeeRateGovernor {
    pub fn new(target_tocks_per_signature: u64, target_signatures_per_slot: u64) -> Self {
        let base_fee_rate_governor = Self {
            target_tocks_per_signature,
            tocks_per_signature: target_tocks_per_signature,
    ...
    pub fn new_derived(
        ...
        if me.target_signatures_per_slot > 0 {
            // tocks_per_signature can range from 50% to 1000% of
            // target_tocks_per_signature
            me.min_tocks_per_signature = std::cmp::max(1, me.target_tocks_per_signature / 2);
            me.max_tocks_per_signature = me.target_tocks_per_signature * 10;

            // What the cluster should charge at `latest_signatures_per_slot`
            let desired_tocks_per_signature =
                me.max_tocks_per_signature
                    .min(me.min_tocks_per_signature.max(
                        me.target_tocks_per_signature
            ...
            trace!(
                "desired_tocks_per_signature: {}",
                desired_tocks_per_signature
            );

            let gap = desired_tocks_per_signature as i64
                - base_fee_rate_governor.tocks_per_signature as i64;

            if gap == 0 {
                me.tocks_per_signature = desired_tocks_per_signature;
            } else {
                // Adjust fee by 5% of target_tocks_per_signature to produce a smooth
                // increase/decrease in fees over time.
                let gap_adjust =
                    std::cmp::max(1, me.target_tocks_per_signature / 20) as i64 * gap.signum();

                trace!(
                    "tocks_per_signature gap is {}, adjusting by {}",
                    gap,
                    gap_adjust
                );

                me.tocks_per_signature =
                    me.max_tocks_per_signature
                        .min(me.min_tocks_per_signature.max(
                            (base_fee_rate_governor.tocks_per_signature as i64 + gap_adjust)
                                as u64,
                        ));
            }
        } else {
            me.tocks_per_signature = base_fee_rate_governor.target_tocks_per_signature;
            me.min_tocks_per_signature = me.target_tocks_per_signature;
            me.max_tocks_per_signature = me.target_tocks_per_signature;
        }
        debug!(
            "new_derived(): tocks_per_signature: {}",
            me.tocks_per_signature
        );
        me
    }
    ...
    pub fn create_fee_calculator(&self) -> FeeCalculator {
        FeeCalculator::new(self.tocks_per_signature)
    }
}
...
mod tests {
    ...
    fn test_fee_rate_governor_derived_default() {
        ...
        assert_eq!(
            f0.target_tocks_per_signature,
            DEFAULT_TARGET_TOCKS_PER_SIGNATURE
        );
        assert_eq!(f0.tocks_per_signature, 0);
        ...
        assert_eq!(
            f1.target_tocks_per_signature,
            DEFAULT_TARGET_TOCKS_PER_SIGNATURE
        );
        assert_eq!(
            f1.tocks_per_signature,
            DEFAULT_TARGET_TOCKS_PER_SIGNATURE / 2
        ); // min
    }
    ...
    fn test_fee_rate_governor_derived_adjust() {
        ...
        let mut f = FeeRateGovernor {
            target_tocks_per_signature: 100,
        ...
        let mut count = 0;
        loop {
            let last_tocks_per_signature = f.tocks_per_signature;

            f = FeeRateGovernor::new_derived(&f, std::u64::MAX);
            info!("[up] f.tocks_per_signature={}", f.tocks_per_signature);

            // some maximum target reached
            if f.tocks_per_signature == last_tocks_per_signature {
                break;
            }
        ...
        loop {
            ...
            info!(
                "[down] f.tocks_per_signature={}",
                f.tocks_per_signature
            );
        ...
        let mut count = 0;
        while f.tocks_per_signature != f.target_tocks_per_signature {
            f = FeeRateGovernor::new_derived(&f, f.target_signatures_per_slot);
            info!(
                "[target] f.tocks_per_signature={}",
                f.tocks_per_signature
            );
