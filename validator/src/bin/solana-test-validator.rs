use {
    ...
    analog_core::tower_storage::FileTowerStorage,
    analog_faucet::faucet::{run_local_faucet_with_port, FAUCET_PORT},
 ...
 * 40MB-150MB range when running `analog-test-validator`
 */
...
fn main() {
    ...
    let matches = App::new("analog-test-validator")
        ...
        .arg(
            Arg::with_name("faucet_anlog")
                .long("faucet-anlog")
                .takes_value(true)
                .value_name("ANLOG")
                .default_value(default_faucet_anlog.as_str())
                .help(
                    "Give the faucet address this much ANLOG in genesis. \
                     If the ledger already exists then this parameter is silently ignored",
                ),
        )
        .get_matches();
    ...
    analog_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        analog_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        analog_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
