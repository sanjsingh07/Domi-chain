...
lazy_static! {
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend(&[".config", "analog", "cli", "config.yml"]);
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend(&[".config", "analog", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.analog.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.analog.com"),
            "ws://api.devnet.analog.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.analog.com"),
            "wss://api.devnet.analog.com/".to_string()
        );
