...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and ANALOG_METRICS_CONFIG is
...
host="https://metrics.analog.com:8086"
...
if $useEnv; then
  [[ -n $ANALOG_METRICS_CONFIG ]] ||
    usage "ANALOG_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  ANALOG_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export ANALOG_METRICS_CONFIG=\"$ANALOG_METRICS_CONFIG\"" >> "$configFile"
