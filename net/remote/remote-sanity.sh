...
local|tar|skip)
  ...
  analog_cli=analog
  analog_gossip=analog-gossip
  analog_install=analog-install
  ;;
*)
...
(
  set -x
  $analog_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $analog_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: analog-install test"
