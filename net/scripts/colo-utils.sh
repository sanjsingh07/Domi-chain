...
declare -r ANALOG_LOCK_FILE="/home/analog/.analog.lock"
...
colo_whoami() {
  ...
  done < <(colo_instance_run_foreach "[ -n \"\${ANALOG_USER}\" ] && echo -e \"\${ANALOG_USER}\\x1fEOL\"")
  echo "${ME}"
}

COLO_ANALOG_USER=""
colo_get_analog_user() {
  if [ -z "${COLO_ANALOG_USER}" ]; then
    COLO_ANALOG_USER=$(colo_whoami)
  fi
  echo "${COLO_ANALOG_USER}"
}
...
__colo_node_status_script() {
  ...
  exec 2>/dev/null  # Suppress stderr as the next call to exec fails most of
                    # the time due to ${ANALOG_LOCK_FILE} not existing and is running from a
                    # subshell where normal redirection doesn't work
  exec 9<"${ANALOG_LOCK_FILE}" && flock -s 9 && . "${ANALOG_LOCK_FILE}" && exec 9>&-
  echo -e "\${ANALOG_LOCK_USER}\\x1f\${ANALOG_LOCK_INSTANCENAME}\\x1f\${PREEMPTIBLE}\\x1fEOL"
