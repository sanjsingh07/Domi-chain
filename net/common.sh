...
source "$ANALOG_ROOT"/scripts/configure-metrics.sh
...
ANALOG_CONFIG_DIR=$ANALOG_ROOT/config
...
_setup_secondary_mount() {
  ...
  (
    ...
    if [[ -d $SECONDARY_DISK_MOUNT_POINT ]] && \
      ...
      rm -rf "$ANALOG_CONFIG_DIR"
      ln -sfT $SECONDARY_DISK_MOUNT_POINT/config "$ANALOG_CONFIG_DIR"
    fi
  )
}
