...
- to have a self-administered gateway to the Analog mainnet-beta cluster to get
...
Analog nodes demand relatively high computing power to handle our fast blocks
and high TPS. For specific requirements, please see
[hardware recommendations](../running-validator/validator-reqs.md).

To run an api node:

1. [Install the Analog command-line tool suite](../cli/install-analog-cli-tools.md)
...
[Current parameters for Mainnet Beta](../clusters.md#example-analog-validator-command-line-2)
...
ensure you miss as little data as possible. Running the analog software as a
...
```bash
analog-watchtower --validator-identity <YOUR VALIDATOR IDENTITY>
```
...
`analog-validator` command to receive historical ledger data instead of a
snapshot.
...
traffic from all other Analog validators. While this is the most efficient mode of
operation, and is strongly recommended, it is possible to restrict the
validator to only require inbound traffic from one other Analog validator.
...
Analog addresses a 32-byte array, encoded with the bitcoin base58 alphabet. This
...
This check is insufficient on its own as Analog addresses are not checksummed, so
...
The address of a normal account in Analog is a Base58-encoded string of a
...
```
$ analog-keygen new -o token-account.json
...
```
$ analog balance 6VzWGL51jLebvnDifvcuEDec17sK6Wupi4gYhm5RzfkV
0
```
...
Be sure to test your complete workflow on Analog devnet and testnet
