...
A front-end application that interacts with a Analog cluster.
...
An increase in token supply over time used to fund rewards for validation and to fund continued development of Analog.
...
Analog's parallel smart contracts run-time.
...
A library of programs on Analog such as spl-token that facilitates tasks such as creating and using tokens
