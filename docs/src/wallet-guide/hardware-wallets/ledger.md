...
This page describes how to use a Ledger Nano S or Nano X to interact with Analog
using the command line tools. To see other solutions to interact with Analog with
your Nano, [click here](../ledger-live.md#interact-with-the-analog-network).

## Before You Begin

- [Set up a Nano with the Analog App](../ledger-live.md)
- [Install the Analog command-line tools](../../cli/install-analog-cli-tools.md)

## Use Ledger Nano with Analog CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the Analog app on the Nano
...
```bash
analog-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the Analog CLI. The command returns your Ledger's unique
...
To view any address, use the `analog-keygen pubkey` command, as shown below,
...
```bash
analog-keygen pubkey usb://ledger
analog-keygen pubkey usb://ledger?key=0
analog-keygen pubkey usb://ledger?key=1
analog-keygen pubkey usb://ledger?key=2
```
...
Any of the addresses displayed by these commands are valid Analog wallet
...
```bash
~$ analog-keygen pubkey usb://ledger?key=42
...
When the URL is not fully qualified, the Analog CLI will prompt you with
...
URLs using the Analog CLI `resolve-signer` command. For example, try
