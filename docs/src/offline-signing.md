...
- Collecting signatures from geographically disparate signers in a
  [multi-signature scheme](https://spl.analog.com/token#multisig-usage)
- Signing transactions using an [airgapped](<https://en.wikipedia.org/wiki/Air_gap_(networking)>)
  signing device

This document describes using Analog's CLI to separately sign and submit a
transaction.
...
- [`create-stake-account`](cli/usage.md#analog-create-stake-account)
- [`deactivate-stake`](cli/usage.md#analog-deactivate-stake)
- [`delegate-stake`](cli/usage.md#analog-delegate-stake)
- [`split-stake`](cli/usage.md#analog-split-stake)
- [`stake-authorize`](cli/usage.md#analog-stake-authorize)
- [`stake-set-lockup`](cli/usage.md#analog-stake-set-lockup)
- [`transfer`](cli/usage.md#analog-transfer)
- [`withdraw-stake`](cli/usage.md#analog-withdraw-stake)
...
```bash
analog@offline$ analog pay --sign-only --blockhash 5Tx8F3jgSHx21CbtjwmdaKPLM5tWmreWAnPrbqHomSJF \
...
```bash
analog@online$ analog pay --blockhash 5Tx8F3jgSHx21CbtjwmdaKPLM5tWmreWAnPrbqHomSJF \
...
```text
analog@offline1$ analog transfer Fdri24WUGtrCXZ55nXiewAj6RM18hRHPGAjZk3o6vBut 10 \
...
```text
analog@offline2$ analog transfer Fdri24WUGtrCXZ55nXiewAj6RM18hRHPGAjZk3o6vBut 10 \
    --blockhash 7ALDjLv56a8f6sH6upAZALQKkXyjAwwENH9GomyM8Dbc \
    --sign-only \
    --keypair from.json \
...
```text
analog@online$ analog transfer Fdri24WUGtrCXZ55nXiewAj6RM18hRHPGAjZk3o6vBut 10 \
    --blockhash 7ALDjLv56a8f6sH6upAZALQKkXyjAwwENH9GomyM8Dbc \
    --from 674RgFMgdqdRoVtMqSBg7mHFbrrNm1h1r721H1ZMquHL \
...
Typically a Analog transaction must be signed and accepted by the network within
