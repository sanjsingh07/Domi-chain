...
Developers can write and deploy their own programs to the Analog blockchain.
...
Analog on-chain programs are compiled via the [LLVM compiler
...
Because Analog uses the LLVM compiler infrastructure, a program may be written
in any programming language that can target the LLVM's BPF backend. Analog
...
The virtual address memory map used by Analog BPF programs is fixed and laid out
as follows
