...
Analog supports writing on-chain programs using the
[Rust](https://www.rust-lang.org/) programming language.

## Project Layout

Analog Rust programs follow the typical [Rust project
...
Analog Rust programs may depend directly on each other in order to gain access
...
At a minimum, Analog Rust programs must pull in the
[analog-program](https://crates.io/crates/analog-program) crate.

Analog BPF programs have some [restrictions](#restrictions) that may prevent the
...
- Crates may depend on `rand` which is not supported in Analog's deterministic
...
To build a specific program, such as SPL Token, for the Analog BPF target which
...
Analog programs can be unit tested via the traditional `cargo test` mechanism by
...
[`program-test`](https://crates.io/crates/analog-program-test) crate. The
...
- String formatting should be avoided since it is also computationally
  expensive.
- No support for `println!`, `print!`, the Analog [logging helpers](#logging)
...
`get-random` support for Analog. The error will typically look like this:
...
```
INFO  analog_runtime::message_processor] Finalized account CGLhHSuWsp1gT4B7MY2KACqp9RUwQRhcUFfVSuxpSajZ
INFO  analog_runtime::message_processor] Call BPF program CGLhHSuWsp1gT4B7MY2KACqp9RUwQRhcUFfVSuxpSajZ
INFO  analog_runtime::message_processor] Program log: Panicked at: 'assertion failed: `(left == right)`
      left: `1`,
     right: `2`', rust/panic/src/lib.rs:22:5
INFO  analog_runtime::message_processor] BPF program consumed 5453 of 200000 units
INFO  analog_runtime::message_processor] BPF program CGLhHSuWsp1gT4B7MY2KACqp9RUwQRhcUFfVSuxpSajZ failed: BPF program panicked
```
