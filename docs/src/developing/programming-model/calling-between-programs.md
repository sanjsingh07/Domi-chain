...
The Analog runtime allows programs to call each other via a mechanism called
...
`invoke()` is built into Analog's runtime and is responsible for routing the
...
Analog, they must each transfer their wager's assets to some intermediary that
will honor their agreement. Currently, there is no way to implement this
intermediary as a program in Analog because the intermediary program cannot
