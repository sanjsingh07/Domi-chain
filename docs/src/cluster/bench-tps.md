...
```bash
./fetch-perf-libs.sh
NDEBUG=1 ANALOG_CUDA=1 ./multinode-demo/bootstrap-validator.sh
NDEBUG=1 ANALOG_CUDA=1 ./multinode-demo/validator.sh
```
...
- To enable BPF program logging:
  ...
  ```bash
  export RUST_LOG=analog_bpf_loader=trace
  ```
...
You can also attach to a running process with GDB. The leader's process is named _analog-validator_:
