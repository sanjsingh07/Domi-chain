...
```bash
analog airdrop 1 <RECIPIENT_ACCOUNT_ADDRESS> --url https://api.devnet.analog.com
```
...
```bash
analog balance <ACCOUNT_ADDRESS> --url https://api.devnet.analog.com
```
...
```bash
analog-keygen new --no-passphrase --no-outfile
```
...
The Analog cluster will only accept the transfer if you sign the transaction
with the private keypair corresponding to the sender's public key in the
transaction.

```bash
analog transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> 0.5 --allow-unfunded-recipient --url https://api.devnet.analog.com --fee-payer <KEYPAIR>
```
...
Confirm the updated balances with `analog balance`:

```bash
analog balance <ACCOUNT_ADDRESS> --url http://api.devnet.analog.com
```
...
```bash
$ analog-keygen new --outfile my_analog_wallet.json   # Creating my first wallet, a file system wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
Wrote new keypair to my_analog_wallet.json
==========================================================================
...
$ analog airdrop 1 DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.analog.com  # Airdropping 1 SOL to my wallet's address/pubkey
Requesting airdrop of 1 SOL from 35.233.193.70:9900
1 SOL

$ analog balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.analog.com # Check the address's balance
1 SOL

$ analog-keygen new --no-outfile  # Creating a second wallet, a paper wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
====================================================================
...
$ analog transfer --from my_analog_wallet.json 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv 0.5 --allow-unfunded-recipient --url https://api.devnet.analog.com --fee-payer my_analog_wallet.json  # Transferring tokens to the public address of the paper wallet
3gmXvykAd1nCQQ7MjosaHLf69Xyaqyq1qw2eu1mgPyYXd5G4v1rihhg1CiRw35b9fHzcftGKKEu4mbUeXY2pEX2z  # This is the transaction signature

$ analog balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.analog.com
...
$ analog balance 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv --url https://api.devnet.analog.com
...
Analog, the wallet address is the public key of a keypair. There are a variety
...
with the `analog transfer` command:

```bash
analog transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> <AMOUNT> --fee-payer <KEYPAIR>
```

Confirm the updated balances with `analog balance`:

```bash
analog balance <ACCOUNT_ADDRESS>
```
