...
in a _stake account_. Analog weights validator votes by the amount of stake
delegated to them, which gives those validators more influence in determining
then next valid block of transactions in the blockchain. Analog then generates
...
```bash
analog-keygen new --no-passphrase -o stake-account.json
```
...
```bash
analog create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `analog stake-account` command:
...
`analog stake-authorize` command. For example, to set a new stake authority,
run:

```bash
analog stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
analog create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
and seed string. To see what stake address the command will derive, use `analog create-address-with-seed`:

```bash
analog create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`analog create-stake-account`.
...
accounts with the `analog validators` command:
...
address in `analog delegate-stake`:

```bash
analog delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `analog stake-account` to observe the changes
...
Once delegated, you can undelegate stake with the `analog deactivate-stake`
command:

```bash
analog deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `analog withdraw-stake` command:

```bash
analog withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `analog split-stake` command:

```bash
analog split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
