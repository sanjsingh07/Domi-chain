...
third at "2", and so on, then the `analog-stake-accounts` tool will allow
...
```bash
analog-stake-accounts new <FUNDING_KEYPAIR> <BASE_KEYPAIR> <AMOUNT> \
...
```bash
analog-stake-accounts count <BASE_PUBKEY>
```
...
```bash
analog-stake-accounts balance <BASE_PUBKEY> --num-accounts <NUMBER>
```
...
```bash
analog-stake-accounts addresses <BASE_PUBKEY> --num-accounts <NUMBER>
```
...
```bash
analog-stake-accounts authorize <BASE_PUBKEY> \
...
```bash
analog-stake-accounts rebase <BASE_PUBKEY> <NEW_BASE_KEYPAIR> \
...
```bash
analog-stake-accounts move <BASE_PUBKEY> <NEW_BASE_KEYPAIR> \
    --stake-authority <KEYPAIR> --withdraw-authority <KEYPAIR> \
    --new-stake-authority <PUBKEY> --new-withdraw-authority <PUBKEY> \
    --num-accounts <NUMBER> --fee-payer <KEYPAIR>
```
