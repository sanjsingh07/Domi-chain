...
This can be seen by explicitly calculating un-staked dilution as $D_{us}$. The un-staked proportion of the token pool at time $t$ is $P_{us}(t_{N})$ and $I_{t}$ is the incremental inflation rate applied between any two consecutive time points. $ANLOG_{us}(t)$ and $ANLOG_{total}(t)$ is the amount of un-staked and total ANLOG on the network, respectively, at time $t$. Therefore $P_{us}(t) = ANLOG_{us}(t)/ANLOG_{total}(t)$.

$$
\begin{aligned}
	D_{us} &= \left( \frac{P_{us}(t_{1}) - P_{us}(t_{0})}{P_{us}(t_{0})} \right)\\
		&= \left( \frac{ \left( \frac{ANLOG_{us}(t_{2})}{ANLOG_{total}(t_{2})} \right) - \left( \frac{ANLOG_{us}(t_{1})}{ANLOG_{total}(t_{1})} \right)}{ \left( \frac{ANLOG_{us}(t_{1})}{ANLOG_{total}(t_{1})} \right) } \right)\\
...
$$
\begin{aligned}
	ANLOG_{us}(t_2) &= ANLOG_{us}(t_1)\\
	ANLOG_{total}(t_2) &= ANLOG_{total}(t_1)\times (1 + I_{t_1})\\
\end{aligned}
$$
...
$$
\begin{aligned}
	D_{us} &= \left( \frac{ \left( \frac{ANLOG_{us}(t_{1})}{ANLOG_{total}(t_{1})\times (1 + I_{1})} \right) - \left( \frac{ANLOG_{us}(t_{1})}{ANLOG_{total}(t_{1})} \right)}{ \left( \frac{ANLOG_{us}(t_{1})}{ANLOG_{total}(t_{1})} \right) } \right)\\
...
As seen in the plot above, the proportion of staked tokens increases with inflation issuance. Letting $ANLOG_s(t)$ and $ANLOG_{\text{total}}(t)$ represent the amount of staked and total ANLOG at time $t$ respectively:

$$
	P_s(t_2) = \frac{ANLOG_s(t_1) + ANLOG_{\text{total}}(t_1)\times I(t_1)}{ANLOG_{\text{total}}(t_1)\times (1 + I(t_1))}\\
$$

Where $ANLOG_{\text{total}}(t_1)\times I(t_1)$ is the additional inflation issuance added to the staked token pool. Now we can write $Y_{adj}$ in common terms $t_1 = t$:

$$
\begin{aligned}
Y_{adj} &= \frac{\frac{ANLOG_s(t) + ANLOG_{\text{total}}(t)\times I(t)}{ANLOG_{\text{total}}(t)\times (1 + I(t))} - \frac{ANLOG_s(t)}{ANLOG_{\text{total}}(t)} }{ \frac{ANLOG_s(t)}{ANLOG_{\text{total}}(t)} }  \\
	&= \frac{ ANLOG_{\text{total}}(t)\times (ANLOG_s(t) + ANLOG_{\text{total}}(t)\times I(t)) }{ ANLOG_s(t)\times ANLOG_{\text{total}}\times (1 + I(t)) } -1 \\
\end{aligned}
$$
