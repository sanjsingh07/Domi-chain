...
account is needed if you plan to run a validator node on Analog.

## Create a Vote Account

A vote account can be created with the
[create-vote-account](../cli/usage.md#analog-create-vote-account) command.
...
the [vote-update-validator](../cli/usage.md#analog-vote-update-validator) command.
...
the transaction fee. Because transaction fees on Analog are assessed
...
using the [withdraw-from-vote-account](../cli/usage.md#analog-withdraw-from-vote-account)
...
[vote-update-commission](../cli/usage.md#analog-vote-update-commission) command.
...
6. After the cluster reaches the next epoch, remove the
   ...
   `analog-validator`, as the old vote authority keypair is no longer required.
