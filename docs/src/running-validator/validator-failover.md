...
### Primary Validator
The following additional `analog-validator` parameters are required to enable
...
following `analog-validator` command-line argument changes:
...
```bash
$ analog-validator wait-for-restart-window --identity validator-keypair.json \
  && analog-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `analog-validator set-identity
