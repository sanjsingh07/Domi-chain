#!/usr/bin/env bash
#
# Fetches the latest SPL programs and produces the analog-genesis command-line
...
fetch_program() {
  ...
  if [[ -r ~/.cache/analog-spl/$so ]]; then
    cp ~/.cache/analog-spl/"$so" "$so"
  else
...
echo
echo "analog-genesis command-line arguments (spl-genesis-args.sh):"
