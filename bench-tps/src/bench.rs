...
fn generate_chunked_transfers(
    ...
    let mut reclaim_tocks_back_to_source_account = false;
    ...
    while start.elapsed() < duration {
        generate_txs(
            ...
            threads,
            reclaim_tocks_back_to_source_account,
        );
        ...
        if chunk_index == 0 {
            reclaim_tocks_back_to_source_account = !reclaim_tocks_back_to_source_account;
        }
    }
}
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    metrics_submit_tock_balance(balance);
...
fn metrics_submit_tock_balance(tock_balance: u64) {
    info!("Token balance: {}", tock_balance);
    datapoint_info!(
        "bench-tps-tock_balance",
        ("balance", tock_balance, i64)
    );
}
...
trait FundingTransactions<'a> {
    ...
    fn verify<T: 'static + Client + Send + Sync>(&mut self, client: &Arc<T>, to_tocks: u64);
}
        ...
        for _ in 0..loops {
            ...
            let verified_set: HashSet<Pubkey> = self
                ...
                .filter_map(move |(k, tx)| {
                    ...
                    let verified = if verify_funding_transfer(&client, tx, to_tocks) {
...
pub fn fund_keys<T: 'static + Client + Send + Sync>(
    ...
    while !not_funded.is_empty() {
        ...
        let to_tocks = (funded_funds - tocks_per_account - max_fee) / MAX_SPENDS_PER_TX;
        for f in funded {
            let start = not_funded.len() - MAX_SPENDS_PER_TX as usize;
            let dests: Vec<_> = not_funded.drain(start..).collect();
            let spends: Vec<_> = dests.iter().map(|k| (k.pubkey(), to_tocks)).collect();
        ...
        funded = new_funded;
        funded_funds = to_tocks;
    }
}

pub fn airdrop_tocks<T: Client>(
    client: &T,
    ...
    metrics_submit_tock_balance(starting_balance);
    ...
    if starting_balance < desired_balance {
        ...
        metrics_submit_tock_balance(current_balance);
...
pub fn generate_and_fund_keypairs<T: 'static + Client + Send + Sync>(
    ...
    // Sample the first keypair, to prevent tock loss on repeated analog-bench-tps executions
    ...
    let enough_tocks = 8 * tocks_per_account / 10;
    if first_keypair_balance < enough_tocks || last_keypair_balance < enough_tocks {
        ...
        let total = tocks_per_account * total_keypairs + extra_fees;

        let funding_key_balance = client.get_balance(&funding_key.pubkey()).unwrap_or(0);
        info!(
            "Funding keypair balance: {} max_fee: {} tocks_per_account: {} extra: {} total: {}",
            funding_key_balance, max_fee, tocks_per_account, extra, total
        );

        if client.get_balance(&funding_key.pubkey()).unwrap_or(0) < total {
            airdrop_tocks(client.as_ref(), &faucet_addr.unwrap(), funding_key, total)?;
        }
...
mod tests {
    ...
    fn test_bench_tps_fund_keys() {
        ...
        let keypairs =
            generate_and_fund_keypairs(client.clone(), None, &id, keypair_count, tocks).unwrap();
    ...
    fn test_bench_tps_fund_keys_with_fees() {
        ...
        for kp in &keypairs {
            assert_eq!(client.get_balance(&kp.pubkey()).unwrap(), tocks);
        }
    }
}
