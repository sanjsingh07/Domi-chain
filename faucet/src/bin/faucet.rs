use {
    ...
    analog_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = analog_cli_config::Config::default().keypair_path;

    analog_logger::setup_with_default("analog=info");
    analog_metrics::set_panic_hook("faucet");
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in ANLOG"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in ANLOG"),
        )
        .arg(
