...
// tocks required to run staking operations for one year
//  the staker account needs carry enough
//  tocks to cover TX fees (delegation) for one year,
//  and we support one delegation per epoch
fn calculate_staker_fees(genesis_config: &GenesisConfig, years: f64) -> u64 {
    genesis_config.fee_rate_governor.max_tocks_per_signature
...
/// create stake accounts for tocks with at most stake_granularity in each
///  account
pub fn create_and_add_stakes(
    ...
    // the largest each stake account should be, in tocks
    ...
    let total_tocks = staker_info.tocks;
    ...
    let mut stakes_tocks = total_tocks - staker_fees;
    ...
    genesis_config
        ...
        .or_insert_with(|| {
            stakes_tocks -= staker_rent_reserve;
    ...
    for unlock in unlocks {
        let tocks = unlock.amount(stakes_tocks);

        let (granularity, remainder) = if granularity < tocks {
            (granularity, tocks % granularity)
        } else {
            (tocks, 0)
        };
        ...
        for _ in 0..(tocks / granularity).saturating_sub(1) {
...
mod tests {
    ...
    fn create_and_check_stakes(
        ...
        assert!(genesis_config
            .accounts
            .iter()
            .all(|(_pubkey, account)| account.tocks <= granularity
                || account.tocks - granularity
    ...
    fn test_create_stakes() {
        ...
        let total_tocks = staker_reserve + reserve * 2 + 1;
        ...
        let total_tocks = staker_reserve + (granularity + reserve) * 2;
        create_and_check_stakes(
            &mut GenesisConfig {
                rent,
                ..GenesisConfig::default()
        ...
        let granularity = reserve * 3;
        let total_tocks = staker_reserve + (granularity + reserve + 1) * 2;
        create_and_check_stakes(
            &mut GenesisConfig {
                rent,
                ..GenesisConfig::default()
