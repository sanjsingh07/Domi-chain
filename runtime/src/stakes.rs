...
impl Stakes {
    ...
    pub fn vote_balance_and_staked(&self) -> u64 {
        self.stake_delegations
            ...
            + self
                .vote_accounts
                .iter()
                .map(|(_pubkey, (_staked, vote_account))| vote_account.tocks())
                .sum::<u64>()
    }
    ...
    pub fn store(
        ...
        if analog_vote_program::check_id(account.owner()) {
            ...
            // when account is removed (tocks == 0 or data uninitialized), don't read so that
            ...
            //  old_stake is stake tocks and voter_pubkey from the pre-store() version
            ...
            let stake = delegation.map(|delegation| {
                (
                    ...
                    if account.tocks() != 0 {
                        ...
                        // when account is removed (tocks == 0), this special `else` clause ensures
            ...
            if remove_delegation {
                // when account is removed (tocks == 0), remove it from Stakes as well
            ...
            // Runtime always enforces an intermediary write of account.tocks == 0,
    ...
    fn test_vote_balance_and_staked_normal() {
        ...
        impl Stakes {
            pub fn vote_balance_and_warmed_staked(&self) -> u64 {
                self.vote_accounts
                    .iter()
                    .map(|(_pubkey, (staked, account))| staked + account.tocks())
                    .sum()
            }
        }
        ...
        for (epoch, expected_warmed_stake) in ((genesis_epoch + 1)..=3).zip(&[2, 3, 4]) {
            ...
            // vote_balance_and_staked() always remain to return same tocks
