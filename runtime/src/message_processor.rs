...
use analog_program_runtime::{ExecuteDetailsTimings, Executors, InstructionProcessor, PreAccount};
use analog_sdk::{
...
pub struct ThisInvokeContext<'a> {
    ...
    compute_budget: ComputeBudget,
    #[allow(deprecated)]
    bpf_compute_budget: analog_sdk::process_instruction::BpfComputeBudget,
...
impl<'a> InvokeContext for ThisInvokeContext<'a> {
    ...
    fn verify_and_update(
                ...
                && index_in_instruction < account_indices.len()
            {
                ...
                for pre_account in pre_accounts.iter_mut() {
                    if key == pre_account.key() {
                        ...
                        pre_sum += u128::from(pre_account.tocks());
                        post_sum += u128::from(account.tocks());
        ...
        // Verify that the total sum of all the tocks did not change
        if pre_sum != post_sum {
    ...
    #[allow(deprecated)]
    fn get_bpf_compute_budget(&self) -> &analog_sdk::process_instruction::BpfComputeBudget {
    ...
    pub fn verify(
        ...
        let (mut pre_sum, mut post_sum) = (0_u128, 0_u128);
        {
            ...
            let mut work = |unique_index: usize, account_index: usize| {
                ...
                pre_sum += u128::from(pre_accounts[unique_index].tocks());
                post_sum += u128::from(account.tocks());
                Ok(())
            };
            instruction.visit_each_account(&mut work)?;
...
mod tests {
    ...
    fn test_process_message_readonly_handling() {
        ...
        fn mock_system_process_instruction(
            ...
            if let Ok(instruction) = bincode::deserialize(data) {
                match instruction {
                    ...
                    MockSystemInstruction::AttemptCredit { tocks } => {
                        keyed_accounts[0]
                            .account
                            .borrow_mut()
        ...
        assert_eq!(result, Ok(()));
        assert_eq!(accounts[0].1.borrow().tocks(), 100);
        assert_eq!(accounts[1].1.borrow().tocks(), 0);

        let message = Message::new(
            &[Instruction::new_with_bincode(
                mock_system_program_id,
                &MockSystemInstruction::AttemptCredit { tocks: 50 },
                account_metas.clone(),
            )],
            Some(&accounts[0].0),
        );
    ...
    fn test_process_message_duplicate_accounts() {
            ...
            if let Ok(instruction) = bincode::deserialize(data) {
                match instruction {
                    MockSystemInstruction::BorrowFail => {
                        ...
                        if from_account.tocks() != dup_account.tocks() {
                    ...
                    MockSystemInstruction::MultiBorrowMut => {
                        ...
                        if from_tocks != dup_tocks {
                            return Err(InstructionError::InvalidArgument);
                        }
                        Ok(())
                    }
                    MockSystemInstruction::DoWork { tocks, data } => {
                        {
        ...
        assert_eq!(result, Ok(()));
        assert_eq!(accounts[0].1.borrow().tocks(), 80);
        assert_eq!(accounts[1].1.borrow().tocks(), 20);
