use {
    ...
    analog_sdk::{clock::Epoch, pubkey::Pubkey},
...
pub(crate) mod tests {
    ...
    use analog_vote_program::vote_state::create_account_with_authorized;
    use std::iter;
    ...
    fn test_parse_epoch_vote_accounts() {
        ...
        let vote_accounts_map: HashMap<Pubkey, Vec<VoteAccountInfo>> = (0..10)
            .map(|_| {
                let node_id = analog_sdk::pubkey::new_rand();
                (
                    node_id,
                    iter::repeat_with(|| {
                        let authorized_voter = analog_sdk::pubkey::new_rand();
                        VoteAccountInfo {
                            vote_account: analog_sdk::pubkey::new_rand(),
