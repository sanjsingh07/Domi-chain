...
fn main() {
    ...
    if let Err(e) = std::fs::remove_file(analog_sys_tuner::ANALOG_SYS_TUNER_PATH) {
        if e.kind() != std::io::ErrorKind::NotFound {
            panic!("Failed to remove stale socket file: {:?}", e)
        }
    }

    let listener = unix_socket::UnixListener::bind(analog_sys_tuner::ANALOG_SYS_TUNER_PATH)
    ...
    if let Some(user) = users::get_user_by_name(&user) {
        ...
        nix::unistd::chown(
            analog_sys_tuner::ANALOG_SYS_TUNER_PATH,
