...
mod tests {
    ...
    fn create_bank(tocks: u64) -> (Bank, Keypair) {
    ...
    fn create_bank_client(tocks: u64) -> (BankClient, Keypair) {
        let (bank, mint_keypair) = create_bank(tocks);
    ...
    fn test_ownable_missing_owner_signature() {
        let mut account_owner_pubkey = analog_sdk::pubkey::new_rand();
        let owner_pubkey = account_owner_pubkey;
        let new_owner_pubkey = analog_sdk::pubkey::new_rand();
    ...
    fn test_ownable_incorrect_owner() {
        ...
        let new_owner_pubkey = analog_sdk::pubkey::new_rand();
        let account = AccountSharedData::new_ref(1, 0, &system_program::id());
        let mallory_pubkey = analog_sdk::pubkey::new_rand(); // <-- Attack! Signing with wrong pubkey
