    ...
    for (i, keyed_account) in keyed_accounts.iter().enumerate() {
        ...
        if is_dup {
            ...
            v.write_all(keyed_account.unsigned_key().as_ref())
                .map_err(|_| InstructionError::InvalidArgument)?;
            v.write_u64::<LittleEndian>(keyed_account.tocks()?)
                .map_err(|_| InstructionError::InvalidArgument)?;
...
pub fn deserialize_parameters_unaligned(
        ...
        .enumerate()
    {
        ...
        if !is_dup {
            ...
            keyed_account
                .try_account_ref_mut()?
                .set_tocks(LittleEndian::read_u64(&buffer[start..]));
            start += size_of::<u64>() // tocks
...
pub fn deserialize_parameters_aligned(
        ...
        .enumerate()
    {
        ...
        if is_dup {
            ...
            account.set_tocks(LittleEndian::read_u64(&buffer[start..]));
            start += size_of::<u64>(); // tocks
...
mod tests {
    ...
    fn test_serialize_parameters() {
        ...
        for ((account, account_info), key) in accounts.iter().zip(de_accounts).zip(keys.clone()) {
            ...
            assert_eq!(account.tocks(), account_info.tocks());
            ...
            assert_eq!(
                (*account_info.tocks.borrow() as *const u64).align_offset(align_of::<u64>()),
                0
            );
            assert_eq!(
                account_info
            ...
            accounts.iter().zip(de_keyed_accounts).zip(keys.clone())
        {
            assert_eq!(key, *de_keyed_account.unsigned_key());
            let account = account.borrow();
            assert_eq!(account.tocks(), de_keyed_account.tocks().unwrap());
            assert_eq!(
                account.data(),
    ...
    pub unsafe fn deserialize_unaligned<'a>(
        ...
        for _ in 0..num_accounts {
            ...
            if dup_info == std::u8::MAX {
                ...
                #[allow(clippy::cast_ptr_alignment)]
                let tocks = Rc::new(RefCell::new(&mut *(input.add(offset) as *mut u64)));
                offset += size_of::<u64>();
