...
use {
    ...
    analog_sdk::{packet::PACKET_DATA_SIZE, pubkey::Pubkey, timing::timestamp},
...
mod test {
    ...
    fn test_prune() {
        ...
        let self_id = analog_sdk::pubkey::new_rand();
        let origin = analog_sdk::pubkey::new_rand();
        ...
        let low_staked_peers = (0..10).map(|_| analog_sdk::pubkey::new_rand());
        ...
        let high_staked_peer = analog_sdk::pubkey::new_rand();
    ...
    fn test_process_push_old_version() {
        let crds = RwLock::<Crds>::default();
        let push = CrdsGossipPush::default();
        let mut ci = ContactInfo::new_localhost(&analog_sdk::pubkey::new_rand(), 0);
        ci.wallclock = 1;
