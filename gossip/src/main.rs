...
use {
    ...
    analog_gossip::{contact_info::ContactInfo, gossip_service::discover},
...
fn parse_gossip_host(matches: &ArgMatches, entrypoint_addr: Option<SocketAddr>) -> IpAddr {
    matches
        ...
        .map(|gossip_host| {
            analog_net_utils::parse_host(gossip_host).unwrap_or_else(|e| {
        ...
        .unwrap_or_else(|| {
            if let Some(entrypoint_addr) = entrypoint_addr {
                analog_net_utils::get_public_ip_addr(&entrypoint_addr).unwrap_or_else(|err| {
                    eprintln!(
