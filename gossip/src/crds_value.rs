use {
    ...
    analog_sdk::sanitize::{Sanitize, SanitizeError},
    ...
    analog_vote_program::vote_transaction::parse_vote_transaction,
    std::{
        borrow::{Borrow, Cow},
        cmp::Ordering,
...
impl SnapshotHashes {
    ...
    pub(crate) fn new_rand<R: Rng>(rng: &mut R, pubkey: Option<Pubkey>) -> Self {
        ...
        let hashes = std::iter::repeat_with(|| {
            ...
            let hash = analog_sdk::hash::new_rand(rng);
            (slot, hash)
        })
        .take(num_hashes)
        .collect();
...
pub struct LegacyVersion {
    pub from: Pubkey,
    pub wallclock: u64,
    pub version: analog_version::LegacyVersion,
}
...
pub struct Version {
    pub from: Pubkey,
    pub wallclock: u64,
    pub version: analog_version::Version,
}
...
impl Version {
    pub fn new(from: Pubkey) -> Self {
        Self {
            from,
            wallclock: timestamp(),
            version: analog_version::Version::default(),
        }
    }
...
mod test {
    use {
        ...
        analog_vote_program::{vote_instruction, vote_state},
