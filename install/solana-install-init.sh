...
# install analog-install. It just does platform detection, downloads the installer
# and runs it.
...
USAGE:
    analog-install-init [FLAGS] [OPTIONS] --data_dir <PATH> --pubkey <PUBKEY>
...
OPTIONS:
    ...
    -u, --url <URL>          JSON RPC URL for the analog cluster
...
main() {
    ...
    *)
    ...
    ensure downloader "$download_url" "$analog_install_init"
    ensure chmod u+x "$analog_install_init"
    if [ ! -x "$analog_install_init" ]; then
        printf '%s\n' "Cannot execute $analog_install_init (likely because of mounting /tmp as noexec)." 1>&2
        printf '%s\n' "Please copy the file to a location where you can execute binaries and run ./analog-install-init." 1>&2
        exit 1
    fi
