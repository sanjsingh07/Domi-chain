...
git clone https://"$GITHUB_TOKEN"@github.com/analog-labs/"$repo_name" .github_export/"$repo_name"
git filter-repo --subdirectory-filter "$subdir" --target .github_export/"$repo_name"
git -C .github_export/"$repo_name" push https://"$GITHUB_TOKEN"@github.com/analog-labs/"$repo_name"
