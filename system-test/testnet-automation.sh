...
function launch_testnet() {
  ...
  if [[ ! -d "${REPO_ROOT}"/net/keypairs ]]; then
    git clone git@github.com:analog-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
  ...
  get_net_launch_software_version_launch_args "$CHANNEL" "analog-release" version_args
...
[[ -n $INFLUX_HOST ]] || INFLUX_HOST=https://metrics.analog.com:8086
...
if [[ -z $ANALOG_METRICS_CONFIG ]]; then
  if [[ -z $ANALOG_METRICS_PARTIAL_CONFIG ]]; then
    echo ANALOG_METRICS_PARTIAL_CONFIG not defined
    exit 1
  fi
  export ANALOG_METRICS_CONFIG="db=$TESTNET_TAG,host=$INFLUX_HOST,$ANALOG_METRICS_PARTIAL_CONFIG"
fi
echo "ANALOG_METRICS_CONFIG: $ANALOG_METRICS_CONFIG"
