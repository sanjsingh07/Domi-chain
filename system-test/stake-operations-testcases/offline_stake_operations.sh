...
if [[ -z "$url" ]]; then
  echo Provide complete URL, ex: "$0" http://api.devnet.analog.com:8899
  exit 1
fi
analog config set --url $url
...
analog-keygen new -o "$dummy_keypair" --no-passphrase --force --silent
analog config set --keypair $dummy_keypair
...
analog-keygen new -o "$online_system_account_keypair" --no-passphrase --force --silent
analog-keygen new -o "$online_nonce_account_keypair" --no-passphrase --force --silent

online_system_account_pubkey="$(analog-keygen pubkey $online_system_account_keypair)"
nonce_account_pubkey="$(analog-keygen pubkey $online_nonce_account_keypair)"
...
analog-keygen new -o "$offline_system_account_keypair" --no-passphrase --force --silent
analog-keygen new -o "$offline_staker_keypair" --no-passphrase --force --silent
analog-keygen new -o "$offline_withdrawer_keypair" --no-passphrase --force --silent
analog-keygen new -o "$offline_custodian_keypair" --no-passphrase --force --silent

offline_system_account_pubkey="$(analog-keygen pubkey $offline_system_account_keypair)"
offline_withdrawer_pubkey="$(analog-keygen pubkey $offline_withdrawer_keypair)"
offline_staker_pubkey="$(analog-keygen pubkey $offline_staker_keypair)"
offline_custodian_pubkey="$(analog-keygen pubkey $offline_custodian_keypair)"

# Airdrop some funds to the offline account.
analog airdrop 100 $offline_system_account_pubkey
analog airdrop 2 $online_system_account_pubkey

# Create a nonce account funded by the online account, with the authority given to the offline account
analog create-nonce-account $online_nonce_account_keypair 1 --nonce-authority $offline_system_account_pubkey --keypair $online_system_account_keypair
nonce="$(analog nonce $nonce_account_pubkey)"

execution_step OFFLINE SYSTEM ACCOUNT BALANCE BEFORE CREATING STAKE ACCOUNTS
(
  set -x
  analog balance $offline_system_account_pubkey
)
...
analog-keygen new -o "$stake_account_keypair" --no-passphrase --force --silent
stake_account_address="$(analog-keygen pubkey $stake_account_keypair)"

sign_only="$(analog create-stake-account $stake_account_keypair 50 \
...
analog create-stake-account $stake_account_keypair 50 \
...
(
  set -x
  analog stake-account $stake_account_address
)
...
analog-keygen new -o $split_stake_account_keypair --no-passphrase --force --silent
split_stake_account_address=$(analog-keygen pubkey $split_stake_account_keypair)

nonce="$(analog nonce $nonce_account_pubkey)"

sign_only="$(analog split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
analog split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
(
  set -x
  analog stake-account $split_stake_account_address
)
...
sign_only="$(analog stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
  $split_stake_account_address --custodian $offline_custodian_keypair --lockup-epoch 0 \
  --keypair $offline_system_account_keypair --sign-only --url http://0.0.0.0)"

signers="$(get_signers_string "${sign_only[@]}")"

analog stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
sign_only="$(analog withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
analog withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
  $split_stake_account_address $offline_system_account_pubkey 10 \
...
vote_account_pubkey="$(awk '{if(NR==4) print $2}'<<<"$(analog show-validators)")"
nonce="$(analog nonce $nonce_account_pubkey)"

# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(analog delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
analog delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(analog deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
# Send the signed transaction on the cluster
analog deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
