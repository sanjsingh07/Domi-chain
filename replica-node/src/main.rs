...
use {
    ...
    analog_runtime::accounts_index::AccountSecondaryIndexes,
    analog_sdk::{exit::Exit, pubkey::Pubkey, signature::Signer},
    analog_streamer::socket::SocketAddrSpace,
    analog_validator::port_range_validator,
    std::{
        collections::HashSet,
        env,
...
pub fn main() {
    ...
    let rpc_bind_address = if let Some(rpc_bind_address) = matches.value_of("rpc_bind_address") {
        analog_net_utils::parse_host(rpc_bind_address).expect("invalid rpc_bind_address")
    } else {
        bind_address
    };
    ...
    let peer_address = analog_net_utils::parse_host(matches.value_of("peer_address").unwrap())
    ...
    let logfile = {
        let logfile = matches
            .value_of("logfile")
            .map(|s| s.into())
            .unwrap_or_else(|| format!("analog-replica-node-{}.log", identity_keypair.pubkey()));
    ...
    let _logger_thread = analog_validator::redirect_stderr_to_file(logfile);
