...
if [[ -z $1 ]]; then
  packages=(--lib --all --exclude analog-local-cluster)
else
  packages=("$@")
fi
...
RUST_LOG=analog=trace _ "$cargo" nightly test --target-dir target/cov --no-run "${packages[@]}"
if RUST_LOG=analog=trace _ "$cargo" nightly test --target-dir target/cov "${packages[@]}" 2> target/cov/coverage-stderr.log; then
  test_status=0
else
  test_status=$?
