...
impl TestValidator {
    ...
    pub fn with_custom_fees(
        mint_address: Pubkey,
        target_tocks_per_signature: u64,
        faucet_addr: Option<SocketAddr>,
        socket_addr_space: SocketAddrSpace,
    ) -> Self {
        TestValidatorGenesis::default()
            .fee_rate_governor(FeeRateGovernor::new(target_tocks_per_signature, 0))
            .rent(Rent {
                tocks_per_byte_year: 1,
                exemption_threshold: 1.0,
                ..Rent::default()
    ...
    fn initialize_ledger(
        ...
        let validator_identity_tocks = anlog_to_tocks(500.);
        let validator_stake_tocks = anlog_to_tocks(1_000_000.);
        let mint_tocks = anlog_to_tocks(500_000_000.);
